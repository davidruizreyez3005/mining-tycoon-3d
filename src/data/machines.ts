...
export function getMachineById(id: string): MachineDef | undefined {
  return MACHINES[id];
}

export function getMachinesByType(type: MachineDef['type']): MachineDef[] {
  return Object.values(MACHINES).filter(m => m.type === type);
}

export function getAvailableMachines(cash: number, depth: number, ownedMachines: string[]): MachineDef[] {
  return Object.values(MACHINES).filter(m => {
    if (!m.unlockRequirement) return true;
...
export function calculateMachineUpgradeCost(machineId: string, currentLevel: number): number {
