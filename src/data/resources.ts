...
export interface ResourceDef {
  ...
  baseValue: number;
  extractionDifficulty: number; // 0-1, higher = slower extraction
  processingRequirement: string | null; // Required machine upgrade
  unlockDepth: number; // Minimum depth to find
  visualMaterial: string; // Material reference
  productionModifiers: {
...
export function getResourcesByRarity(rarity: ResourceDef['rarity']): ResourceDef[] {
  return Object.values(RESOURCES).filter(r => r.rarity === rarity);
}

export function getAvailableResourcesAtDepth(depth: number): ResourceDef[] {
  return Object.values(RESOURCES).filter(r => r.unlockDepth <= depth);
}

export function calculateResourceValue(resourceId: string, modifiers: { valueMultiplier?: number } = {}): number {
