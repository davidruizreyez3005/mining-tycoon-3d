...
import { calculateProductionRate, calculateOfflineEarnings, validateOfflineTime, formatMoney } from '../../src/systems/economySystem.js';
...
const diamond = RESOURCES['diamond'];
...
const pickaxe = MACHINES['pickaxe'];
...
const crusher_t1 = MACHINES['crusher_t1'];
...
const novice_miner = WORKERS['novice_miner'];
...
// Test formatMoney
assertEqual(formatMoney(100), '$100', 'Format $100');
assertEqual(formatMoney(1500), '$1.50K', 'Format $1.5K');
assertEqual(formatMoney(2500000), '$2.50M', 'Format $2.5M');
assertEqual(formatMoney(3500000000), '$3.50B', 'Format $3.5B');

// Test offline time validation
const now = Date.now();
const oneHourAgo = now - (60 * 60 * 1000);
const validation = validateOfflineTime(oneHourAgo);
...
const futureValidation = validateOfflineTime(future);
...
const oldValidation = validateOfflineTime(thirtyDaysAgo);
...
const result = calculateProductionRate(basicState, 1); // 1 minute
...
const offlineResult = calculateOfflineEarnings(mockSaveData as any, 60);
...
const fullResult = calculateProductionRate(fullStorageState, 1);
