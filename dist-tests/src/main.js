...
// Game state
const gameState = (0, gamestate_1.createGameState)();
let saveData = null;
...
function handleMineAction() {
    ...
    const resource = resources_1.RESOURCES['stone'];
    const amount = 1;
    // Add to inventory
...
function loadOrCreateSave() {
    if ((0, saveSystem_1.hasSave)()) {
        saveData = (0, saveSystem_1.loadGame)();
        if (saveData) {
            // Calculate offline earnings
            const timeValidation = (0, economySystem_1.validateOfflineTime)(saveData.timestamp);
            if (timeValidation.valid && timeValidation.minutes > 1) {
                const offlineResult = (0, economySystem_1.calculateOfflineEarnings)(saveData, timeValidation.minutes);
                if (offlineResult.moneyEarned > 0) {
                    ...
                    setTimeout(() => {
                        showNotification(`While you were away:\nEarned ${(0, economySystem_1.formatMoney)(offlineResult.moneyEarned)}`);
                    }, 1000);
                }
            }
        }
        ...
        saveData = (0, saveSystem_1.createNewSave)();
    }
    lastSaveTime = Date.now();
}
function autoSave() {
    if (saveData && gameState.currentState === 'PLAYING') {
        (0, saveSystem_1.saveGame)(saveData);
...
function updateHUD() {
    ...
    if (moneyDisplay)
        moneyDisplay.textContent = (0, economySystem_1.formatMoney)(saveData.player.cash);
    if (depthDisplay)
        depthDisplay.textContent = `Depth: ${saveData.world.currentDepth}m`;
